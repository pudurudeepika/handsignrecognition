...
# Variables to manage predictions and sentence
sentence = ""
last_letter = ""
letter_buffer = deque(maxlen=15)

# Function to check if all 5 fingers (except thumb) are up
def fingers_up(hand_landmarks):
    tips = [4, 8, 12, 16, 20]
    fingers = []
    for i in range(1, 5):  # Ignore thumb
        if hand_landmarks.landmark[tips[i]].y < hand_landmarks.landmark[tips[i] - 2].y:
            ...
            fingers.append(0)
    return sum(fingers) == 4  # all 4 fingers up = space
